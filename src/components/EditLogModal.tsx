...
export default function EditLogModal({ logToEdit, onClose, onSuccess }: { logToEdit: LogEntry, onClose: () => void, onSuccess: () => void }) {
  const { profile } = useAuth();
  
  const [employeeName, setEmployeeName] = useState(logToEdit.employeeName);
  const [activity, setActivity] = useState(logToEdit.activity);
  const [date, setDate] = useState(logToEdit.date);
  const [field, setField] = useState(logToEdit.field);
  const [timeStart, setTimeStart] = useState(logToEdit.timeStart);
  const [timeEnd, setTimeEnd] = useState(logToEdit.timeEnd);
  const [summary, setSummary] = useState(logToEdit.summary || "");
  const [tagsInput, setTagsInput] = useState(logToEdit.tags ? logToEdit.tags.join(", ") : "");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let audioPath = logToEdit.audioPath;
      if (audioFile) {
      ...
      const lat = selectedField ? selectedField.center.lat : logToEdit.lat;
      const lng = selectedField ? selectedField.center.lng : logToEdit.lng;
      ...
      const { error: updateError } = await supabase.from("logs").update({
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#fff", padding: "28px", borderRadius: "16px", width: "100%", maxWidth: "500px", fontFamily: "var(--font-body)", maxHeight: "90vh", overflowY: "auto" }}>
        <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", margin: "0 0 20px 0" }}>Edit Activity Log</h2>
